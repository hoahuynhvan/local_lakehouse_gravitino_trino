# Local Lakehouse with Apache Gravitino
This project is a fork of the [vutrinh274/local_lakehouse] repository. It provides a complete, containerized Data Lakehouse environment for local development and experimentation.

The core update in this version is the replacement of Project Nessie with Apache Gravitino as the central metadata lakehouse catalog.

# 🏗 Architecture Overview
This stack implements a Medallion Architecture (Bronze → Silver → Gold) using the following open-source components:

• Storage: MinIO(https://min.io/) (S3-compatible object storage).
• Table Format: Apache Iceberg(https://iceberg.apache.org/).
• Catalog Management: Apache Gravitino(https://gravitino.apache.org/) (Replacing Project Nessie). Gravitino provides a unified metadata lake for managing datasets across different stacks.
• Query Engine: Trino(https://trino.io/) (Distributed SQL query engine).
• Transformation: dbt (data build tool)(https://www.getdbt.com/) (SQL-based transformations).
• Orchestration: Apache Airflow(https://airflow.apache.org/) (Workflow scheduling and management).

# 🚀 Key Changes: Nessie vs. Gravitino
While Project Nessie focuses on Git-like versioning for Iceberg tables, Apache Gravitino acts as a federated catalog. By switching to Gravitino, this project now supports:

• Unified Metadata: A single point of control for multiple catalogs.
• Role-Based Access Control: Advanced security management.
• Metalake Abstraction: High-level grouping of catalogs for multi-tenancy.
...
1. Clone the repository: `git clone <your-repo-url>` `cd local_lakehouse`
2. Spin up the services: `./manage-lakehouse.sh up`
3. Verify Gravitino: Access the Gravitino Web UI at `http://localhost:8090`.
4. Run dbt Pipelines: `dbt seed` `dbt run`

# 📊 Medallion Layers
• Landing (Bronze): Raw CSV files loaded via dbt seeds.
• Staging (Silver): Cleaned and standardized data.
• Curated (Gold): Business-ready dimensional models.

# 🤝 Acknowledgments
Original project structure and logic by [vutrinh274].
